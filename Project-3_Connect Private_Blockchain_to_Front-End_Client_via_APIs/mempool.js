...
class Mempool {
  constructor(app) {
    this.app = app;
    this.chain = new Database(`${DB_LOCATION}/chain`);
    this.mempool = new Database(`${DB_LOCATION}/mempool`);;
    this.requests = new Database(`${DB_LOCATION}/requests`);;
    this.initBlockchain();
    this.getBlockByHash();
  ...
  async createBlock({ address, star }) {
    ...
    //}
    block.previousBlockHash = chainData[chainData.length - 1].value.hash;
    // set hash
...
module.exports = (app) => new Mempool(app);
